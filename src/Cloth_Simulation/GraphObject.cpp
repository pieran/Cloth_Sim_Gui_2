...
GraphObject::GraphObject()
{
	...
	m_GraphPosX = width - m_Params.graph_width - 10;
	m_GraphPosY = 10;
}
...
void GraphObject::OnUpdateObject(float dt)
{
	...
	if (m_Visible)
	{
		...
		Matrix4 invProjView = Matrix4::Inverse(SceneManager::Instance()->GetProjViewMatrix());
		Matrix4 ortho = Matrix4::Orthographic(-1.0f, 1.0f, width, 0, 0, height);
...
void GraphObject::DrawXAxis()
{
	...
	for (uint i = 0; i < max_itr; ++i)
	{
		...
		NCLDebug::DrawTextCs(projTransformGraph * Vector4(x_offset, m_Params.graph_height + m_Params.axis_x_label_offset, 0, 1), m_Params.axis_x_label_fontsize, ss.str(), TEXTALIGN_CENTRE, colour);
...
void GraphObject::DrawYAxis()
{
	...
	for (i = 0; i <= max_itr; ++i)
	{
		...
		NCLDebug::DrawTextCs(projTransformGraph * label_pos, m_Params.axis_y_label_fontsize, ss.str(), TEXTALIGN_RIGHT, m_Params.axis_y_label_colour);
	}
}
...
void GraphObject::DrawSeriesLabels()
{
	...
	for (uint idx = 0; idx < m_Series.size(); ++idx)
	{
		...
		NCLDebug::DrawTextCs(projTransform * Vector4(label_x, pos_y, m_Params.series_depth, 1.0f), m_Params.series_labels_fontsize, series->Label(), TEXTALIGN_LEFT, m_Params.series_labels_colour);
...
void GraphObject::DrawGraphTitle()
{
	...
	NCLDebug::DrawTextCs(projTransform * Vector4(pos_x, pos_y, m_Params.title_depth, 1.0f), m_Params.title_text_fontsize, m_Params.title_text, TEXTALIGN_CENTRE, m_Params.title_text_colour);
}
