...
Matrix4::Matrix4(const Matrix3& mat33, const Vector3& position)
{
...
void Matrix4::SetMatrix33(const Matrix3& in)
{
	const unsigned int size = 3 * sizeof(float);
	memcpy(&values[0], &in.mat_array[0], size);
	memcpy(&values[4], &in.mat_array[3], size);
	memcpy(&values[8], &in.mat_array[6], size);
}
