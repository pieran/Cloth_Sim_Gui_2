...
void SceneManager::EnqueueScene(Scene* scene)
{
	...
	if (m_AllScenes.size() == 1)
		...
		Window::SetWindowTitle("NCLTech - [%d/%d] %s", m_SceneIdx + 1, m_AllScenes.size(), m_Scene->GetSceneName().c_str());
}
...
void SceneManager::JumpToScene(int idx)
{
	...
	Window::SetWindowTitle("NCLTech - [%d/%d] %s", idx + 1, m_AllScenes.size(), m_Scene->GetSceneName().c_str());
}
