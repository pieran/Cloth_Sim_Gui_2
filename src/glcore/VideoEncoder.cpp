...
void VideoEncoder::BeginEncoding(const std::string& filename)
{
	if (fmt) EndEncoding();
	...
	glBindBuffer(GL_PIXEL_PACK_BUFFER, m_CopyPixelBuffer);
	glBufferData(GL_PIXEL_PACK_BUFFER, width * height * 3, NULL, GL_STATIC_READ);
